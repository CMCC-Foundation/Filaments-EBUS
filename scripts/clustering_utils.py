...
def generate_mask_time_series(delta_chl, delta_sst, 
    ...
    Input: 
        ...
        filter_filaments_cluster(optional, default to True) : if true, generates a binary mask for filaments delimitation
    Output:
...
def find_filament_cluster(train_data, labels):
    """Finds the filament cluster (associated to greatest chlorophyll anomaly)
    Input:
        train_data (pandas.DataFrame) : fitted train dataset 
        labels (np.array) : labels array
    Output:
        cluster_number (int) : filaments cluster number"""
