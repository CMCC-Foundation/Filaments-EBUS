...
def map_colors_to_labels(train_data, labels):
    ...
    cold_cluster = cluster_labels[labels[np.argmin(train_data["sst"])]]
    filament_cluster = cluster_labels[labels[np.argmax(train_data["chl"])]]
    warm_cluster = cluster_labels[labels[np.argmax(train_data["sst"])]]
    if filament_cluster == cold_cluster:
        filtered_train = train_data.iloc[labels != labels[np.argmax(train_data["chl"])]]
        filtered_labels = labels[labels!=labels[np.argmax(train_data["chl"])]]
        cold_cluster = cluster_labels[filtered_labels[np.argmin(filtered_train["sst"])]]
    
    # Identify the last remaining cluster
    all_clusters = set({'A','B','C','D'})
...
def plot_clustered_space(train_data, labels, pipeline, axs, fontsize = 13):
    ...
    for c, i in cols.items():
        if c == 'green':
            ...
            axs["B"].text(-3, -6, str(i), ha = "center", va = "center", size = 15, 
                          color = c, path_effects=[pe.withStroke(linewidth=1, foreground="black")])
        elif c == 'salmon':
            axs["B"].text(-20, 4, str(i), ha = "center", va = "center", size = 15, 
                          color = c, path_effects=[pe.withStroke(linewidth=1, foreground="black")])
    ...
    axs["D"].set_xticks(range(0, len(perc_counts)), labels = ['A', 'B', 'C', 'D'])
